...
const AdminEditPage = async ({ params }: AdminEditPageProps) => {
  ...
  if (!talkResponse.data.success) notFound();

  const talk = talkResponse.data.talk;

  const conferenceResponse = await api.get<ConferenceResponse>(
    `/conferences/${talk.conferenceId}`
  );
  ...
  return (
    <AdminEditProvider
      talk={{ initialTalk: talk }}
      conference={{ initialConference: conference }}
    >
      <EditTalkForm />
    </AdminEditProvider>
  );
