...
const Home = () => {
  ...
  return (
    <div className='m-auto flex flex-col gap-12 text-center'>
      <div ref={selectRef} className='relative min-w-lg' onClick={selectMenu.open}>
        <form className='flex max-w-lg flex-col gap-4' action={analyzeConferenceAction}>
                ...
                <Link href={ROUTES.TALK(talk.id)}>
                  <div className='flex items-center gap-4 rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-white/10'>
                    ...
                    <div className='flex flex-col items-start justify-start gap-1 overflow-hidden'>
                      ...
                      <div className='text-sm'>{talk.speakers[0].name}</div>
                    </div>
                  </div>
                </Link>
              </div>
